...
optimParallelMP2 <- function(par, fn, gr = NULL, ..., 
...
#    e <- list2env(dots)
#    e <- new.env()
#save(dots, file="debug")
#save(fixed.pars, file="debug1")
#save(e, file="debug2")
#readline("3b")
...
getExprApprox <- function(n, fn, name, dots){
    ...
    ex <- paste0(ex, getFunCallStr(fn, dots=dots), "})")
    parse(text=ex)
}
